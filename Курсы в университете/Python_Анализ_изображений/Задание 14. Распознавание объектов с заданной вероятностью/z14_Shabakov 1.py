...
#Функция проверки типа файла
    ...
    def open_result(self):
        if self.file_check=='Image':
            os.startfile(os.path.join(self.execution_path, self.save_dir,'result' + '[' + os.path.basename(self.file_path) + ']'+'.jpg'))
        if self.file_check == 'Video':
            os.startfile(os.path.join(self.execution_path,  self.save_dir,'result' + '[' + os.path.basename(self.file_path) + ']'+'.avi'))
    ...
    def start_detection(self):
        ...
        if self.file_check == 'Video':
            ...
            self.list = detector.detectObjectsFromVideo(input_file_path=self.file_path,
                                                        output_file_path=os.path.join(self.execution_path, self.save_dir,
        ...
        if self.file_check == 'Image':
            ...
            self.list = detector.detectObjectsFromImage(input_image=self.file_path,
                                                        output_image_path=os.path.join(self.execution_path, self.save_dir,
                                                                                      'result' + '[' + os.path.basename(
                                                                                          self.file_path) +']'+'.jpg'),
                                                        minimum_percentage_probability=self.minProb,
                                                        display_percentage_probability=self.prob,
